...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (proc-exp (vars body)
                (let ((frees (free-variables vars body '())))
                  (proc-val (procedure vars body (optimize-env env frees)))))
...
(define free-variables
  (lambda (bound-vars expr bound)
    (cases expression expr
      (const-exp (num) '())
      (nullary-exp (op) '())
      (unary-exp (op exp1)
                 (free-variables bound-vars exp1 bound))
      (binary-exp (op exp1 exp2)
                  (append (free-variables bound-vars exp1 bound)
                          (free-variables bound-vars exp2 bound)))
      (n-ary-exp (op exprs)
                 (append (flatten (map (lambda (var)
                                        (free-variables bound-vars var bound))
                                       exprs))))
      (var-exp (var)
               (if (or (memq var bound)
                       (memq var bound-vars))
                   '()
                   (list var)))
      (if-exp (pred consq alte)
              (append (free-variables bound-vars pred bound)
                      (free-variables bound-vars consq bound)
                      (free-variables bound-vars alte bound)))
      (cond-exp (lhss rhss)
                (append (free-variables bound-vars lhss bound)
                        (free-variables bound-vars rhss bound)))
      (let-exp (var value body)
               (append (free-variables bound-vars value bound)
                       (free-variables bound-vars body (cons var bound))))
      (let*-exp (vars exprs body)
      ...
      (unpack-exp (vars expr body)
                  (flatten (map (lambda (var)
                                ...
                                vars)))
      (proc-exp (vars body)
                (append (free-variables bound-vars body (cons vars bound))))
      (letproc-exp (name vars proc-body exp-body)
                   (flatten (map (lambda (var)
                                   (free-variables bound-vars var bound))
                                 vars)))
