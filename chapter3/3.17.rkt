...
(define extend-env-let
  (lambda (vars vals env)
    (cond ((null? vars) env)
          (else (extend-env-let (cdr vars)
                                (cdr vals)
                                (extend-env (car vars)
                                            (car vals)
                                            env))))))

;; generate new env with evaluating expressions one by one
(define extend-env-let*
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (let-exp (vars1 exps1 body)
        (let ((vals1 (value-of-let-exp exps1 env)))
          (value-of body (extend-env-let vars1 vals1 env))))
      (let*-exp (vars1 exps1 body)
        (let ((env1 (extend-env-let* vars1 exps1 env)))
          (value-of body env1))))))

;; evaluate expressions in cond clause
...
;; evaluate expression in assignments of let clause
