...
(define build-list
  (lambda (num proc)
    (letrec ((helper (lambda (i num proc)
                       (cond
                         ((= i num) '())
                         (else (cons (proc i) (helper (+ i 1) num proc)))))))
      (helper 0 num proc))))
...
(define extend-env-rec*
  (lambda (p-names b-vars bodys saved-env)
    (let ((vecs (build-list (length p-names) (lambda (x) (vector-val (make-vector 1))))))
      (let ((new-env (extend-env* p-names vecs saved-env)))
        (map (lambda (vars body vec)
               (vector-set! (expval->vector vec) 0
                             ...
                             (lambda (vars vals env)
                               (cond
                                 ...
                                 ((equal? (car vars) search-var)
                                  (let ((result (car vals)))
                                    (cases expval result
                                      (vector-val (vec) (vector-ref (expval->vector result) 0))
                                      (else result))))
                                 (else (loop (cdr vars)
                                             (cdr vals)
                                             env))))))
...
(define scanner-spec
  '((whitespace (whitespace) skip)
    ...
    (unary-op ((or "zero?" "minus" "car" "cdr" "null?" "print" "add1")) string)
...
(define unary-table
  (make-table (entry "zero?" (unary-arithmetic-predicate zero?))
              ...
              (entry "print" print)
              (entry "add1" (unary-arithmetic-func (lambda (x) (+ x 1))))))
