    ...
    (cases expression exp
      ...
      (proc-exp (vars body)
                (let ((frees (free-variables body '())))
                  (proc-val (procedure vars body (optimize-env env frees) #f))))
      (traceproc-exp (vars body)
                     (let ((frees (free-variables body '())))
                       (proc-val (procedure vars body (optimize-env env frees) #t))))
      (letproc-exp (name vars proc-body exp-body)
                   (let ((p-val (proc-val (procedure vars proc-body env #f))))
